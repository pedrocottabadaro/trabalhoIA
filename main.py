...
class Grafo(): #grafo
    ...
    def inserirNoNaSolucao(self, no):
        ...
        if self.verificaSeExiste(no):
            ...
            self.setNoAtual(no)
            self.vetoresFechados.append(no.vetorJarros)
            print(no.vetorJarros[0])
            print(no.vetorJarros[1])
            print(no.vetorJarros[2])
    def verificaSolucao(self):

        auxVet=self.noAtual.vetorJarros

        if(auxVet[0].getCapacidade()%2==0):
            return auxVet[0].getQuantidadeAtual()==auxVet[0].getCapacidade()/2
        else:
            return auxVet[0].getQuantidadeAtual()==(auxVet[0].getCapacidade()/2)+1
